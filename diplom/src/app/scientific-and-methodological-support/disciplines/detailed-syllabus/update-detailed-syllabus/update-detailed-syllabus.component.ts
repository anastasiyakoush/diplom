...
export class UpdateDetailedSyllabusComponent implements OnInit {
  ...
  ngOnInit() {
    ...
    this.endpointService.getDoctype().subscribe(data=> this.doctypes = data)
  }
  ...
  save() {
    this.endpointService.CreateDocByDiscId(this.form).subscribe(()=>this.saveClick.emit())
